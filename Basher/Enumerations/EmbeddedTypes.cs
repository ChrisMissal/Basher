...
namespace Basher.Enumerations
{
    internal class EmbeddedTypes : Enumeration<EmbeddedTypes, ulong>
    {
        ...
        //public static EmbeddedTypes CNETMsg_SetConVar = new EmbeddedTypes(6, global::CNETMsg_SetConVar.Descriptor);
        ...
        public T Read<T>(CDemoPacket packet) where T : IMessage<T>
        {
            packet.Dump(ConsoleColor.Magenta);

            /*var value = this.Descriptor.Parser.ParseFrom(stream);
            value.MergeDelimitedFrom(stream);

            value.Dump(ConsoleColor.Cyan);
